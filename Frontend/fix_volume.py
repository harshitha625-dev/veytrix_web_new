...
def main():
    ...
    # Insert after setClipSettings
    target = '    const [clipSettings, setClipSettings] = useState<Record<string, any>>({});'
    replacement = target + callback_block
    
    if target in content:
        content = content.replace(target, replacement)
    else:
        print("Could not find setClipSettings target!")
